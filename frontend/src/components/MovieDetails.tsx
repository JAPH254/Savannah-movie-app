...
export default function MovieDetailsModal() {
  ...
  const closeModal = () => {
    navigate("/"); // go back to homepage (or previous page)
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center text-gray-800 justify-center bg-black/60">
      {/* Modal Card */}
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div>
            ...
            <p className="text-gray-500">
              Released: {selected.release_date} ({selected.status})
            </p>
            {selected.tagline && (
            ...
            <ul className="mt-4 text-sm text-gray-700 space-y-1">
              <li><strong>Runtime:</strong> {selected.runtime} min</li>
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {selected.credits.cast.slice(0, 8).map((actor:any) => (
                <div key={actor.cast_id} className="bg-white rounded-lg shadow p-2">
                  {actor.profile_path && (
                    <img
